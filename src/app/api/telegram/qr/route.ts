import { NextResponse, type NextRequest } from "next/server";
import { getTelegramManager } from "../../../../lib/telegram.js";
import { requireAppUser } from "../../../../auth/guard.js";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

// GET /api/telegram/qr → the pending tg://login payload for the frontend to
// render, or null. Short-lived auth material: in-memory only, cleared when
// invalid, never logged or persisted.
export async function GET(request: NextRequest): Promise<NextResponse> {
  const auth = await requireAppUser(request);
  if (auth instanceof NextResponse) return auth;
  const manager = getTelegramManager(auth.id);
  return NextResponse.json({ qr: manager.getQRCode() });
}
